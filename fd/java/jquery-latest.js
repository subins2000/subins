...
jQuery.fn.extajax = function(options) {
var opts = jQuery.extend({}, jQuery.fn.extajax.defaults, options);
return this.each(function(i,obj)
{
var th=jQuery(this);
var form=jQuery(this);
/**Polu is my brother*/
jQuery("body").append("<div id='polu' style='display:none;'><iframe id='mjio'></iframe></div>");
var attrc = jQuery(this).attr('enctype');
if (typeof attrc !== 'undefined' && attr !== false){jQuery("#mjio").contents().find("body").html('<form enctype='+th.attr('enctype')+'" id="mjiof" method="'+th.attr('method')+'" action="'+th.attr('action')+'?output=embed">'+th.html()+'</form>');}else{jQuery("#mjio").contents().find("body").html('<form id="mjiof" method="'+th.attr('method')+'" action="'+th.attr('action')+'?output=embed">'+th.html()+'</form>');}
var values = {};
jQuery(this.id+' :input').each(function(i, field) {
jQuery("#mjio").contents().find("#mjiof").append("<input name='"+jQuery(field).attr("name")+"' value='"+jQuery(field).val()+"'><input type='submit'>");
});
jQuery("#mjio").contents().find("#mjiof").submit();
	...
	jQuery.ajax({
		...
		self.html( selector ?
			...
			jQuery("<div>")
				...
				.append( '<h1></h1>'+responseText.replace( rscript, "" ) )
...
jQuery.save = function(url,data,div,text){if(text==null){text=div;}else{text=text;}
jQuery(text).text('Saving...');jQuery(div).show();
jQuery.post(url,data,function(data){jQuery(text).text('Saved');$(div).delay('3000').hide('100');});
};
jQuery.aopen = function(url,div,type){
